    ...
    if (campoObrig.length > 0) {
        ...
        campoObrig.forEach(campo => {
            ...
            if (campoType === "radio") {
                ...
                // Destaca o campo
                $(campo).siblings("span").children(":first").children().css({ "background-color": "#f2c8cb" });

                // Remove destaque quando o campo é alterado e não está vazio
                $(`[name="${campoName}"]`).change(function () {
                    if ($(this).val()) {
                        $(this).siblings("span").children(":first").children().css({ "background-color": "white" });
                    }
                });
                ...
                // Destaca o campo
                $(campo).css({ "background-color": "#f2c8cb" });
                $(campo).parents('div.bootstrap-tagsinput').css("background-color", "#f2c8cb"); //autocomplete

                // Remove destaque quando o campo perde o foco e não está vazio
                //autocomplete
                $(campo).parents('div.bootstrap-tagsinput').on("click", function () {
                    $(".bootstrap-tagsinput").css({ "background-color": "" })
                });
                $(campo).blur(function () {
                    if ($(this).val()) {
